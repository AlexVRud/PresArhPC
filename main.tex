...
\begin{frame}\frametitle{Команды языка Ассемблер: mov}
  \begin{block}{Прототип}
    \cl{mov \{назначение\}, \{источник/значение\}}
  \end{block}
  \begin{block}{}
    \begin{itemize}
      \item Команда копирования.
    \end{itemize}
  \end{block}
...
\only<1,2,3,4>{\node[databox=1,text=brown] (EAX) at (0,3*\ByteHeight) {?};}
\only<5>{\node[databox=1,text=brown,fill=green!30!white] (EAX) at (0,3*\ByteHeight) {32};}
\only<6>{\node[databox=1,text=brown,fill=green!30!white] (EAX) at (0,3*\ByteHeight) {75};}
\only<7,8,9>{\node[databox=1,text=brown] (EAX) at (0,3*\ByteHeight) {75};}
%
\node[left=0cm of EAX] (leax) {EAX};
\node[below=2.84*\ByteHeight of leax,inner sep=1pt] (EIP) {EIP};
%
\only<1>{\draw [->,draw=violet,thick] (EIP) |- (-8pt,-1*0.5*\baselineskip+0.25*\baselineskip);}
\only<2>{\draw [->,draw=violet,thick] (EIP) |- (-8pt,-3*0.5*\baselineskip+0.25*\baselineskip);}
\only<3>{\draw [->,draw=violet,thick] (EIP) |- (-8pt,-7*0.5*\baselineskip+0.25*\baselineskip);}
\only<4>{\draw [->,draw=violet,thick] (EIP) |- (-8pt,-8*0.5*\baselineskip+0.25*\baselineskip);}
\only<5>{\draw [->,draw=violet,thick] (EIP) |- (-8pt,-10*0.5*\baselineskip+0.25*\baselineskip);}
\only<6>{\draw [->,draw=violet,thick] (EIP) |- (-8pt,-11*0.5*\baselineskip+0.25*\baselineskip);}
\only<7>{\draw [->,draw=violet,thick] (EIP) |- (-8pt,-12*0.5*\baselineskip+0.25*\baselineskip);}
\only<8>{\draw [->,draw=violet,thick] (EIP) |- (-8pt,-4*0.5*\baselineskip+0.25*\baselineskip);}
\only<9>{\draw [->,draw=violet,thick] (EIP) |- (-8pt,-5*0.5*\baselineskip+0.25*\baselineskip);}
%
...
\begin{scope}[xshift=-1*\ByteWidht,yshift=1*\ByteHeight,scale=0.6, every node/.style={scale=0.6}]
  ...
  \only<3,4,5,6,7,8,9>{%
    ...
    \node[databox=4,fill=green!30!white,right=-\pgflinewidth of tmp] (data) {return ptr};
    \node[databox=4,right=-\pgflinewidth of data] (data) {32};
...
\end{scope}
\draw [->,draw=violet,thick] (ESP) to[out=-90,in=90] (headofstack);
\begin{pgfonlayer}{background}
