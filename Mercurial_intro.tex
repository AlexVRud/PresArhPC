...
\node[recept, rounded corners, fill=white, below=1cm of p1milk] (p1copy) {\small Копирование};

\node[recept, align=center, fill=green!50, below=1cm of p1copy] (p21t) {Меню};
\node[recept, fill=green!20, below=-1pt of p21t] (p21milk) {Молоко};

\node[recept, rounded corners, fill=blue!20, right of=p1t] (p1to2soup) {+Суп};

\node[recept, rounded corners, fill=red!20, right of=p21t] (p2to3omelette) {+Омлет};
\node[rectangle split, align=left, rectangle split parts=2, rectangle split part fill={green!20, yellow!20}, draw, text width =2cm, rounded corners, below=2pt of p2to3omelette] (p3to4) {-Молоко \nodepart{two} +Чай};

\node[recept, rounded corners, fill=white, right=5cm of p1copy] (p3merge) {\small Слияние};

\node[recept, align=center, fill=green!50, right of=p3merge, yshift=2cm] (p4t) {Меню};
\node[recept, fill=yellow!20, below=-1pt of p4t] (p4tee) {Чай};
\node[recept, fill=blue!20, below=-1pt of p4tee] (p4soup) {Суп};
\node[recept, fill=red!20, below=-1pt of p4soup] (p4omelette) {Омлет};

\node[recept, align=center, fill=green!50, below=1.4cm of p4soup] (p24t) {Меню};
\node[recept, fill=yellow!20, below=-1pt of p24t] (p24tee) {Чай};
\node[recept, fill=red!20, below=-1pt of p24tee] (p24omelette) {Омлет};
\node[recept, fill=blue!20, below=-1pt of p24omelette] (p24soup) {Суп};

\draw[-o] (p1milk) -- (p1copy);
\draw[->] (p1copy) -- (p21t);

\draw[-o] (p1t) -- (p1to2soup);
\draw[-o] (p21t) -- (p2to3omelette);
\draw[->] (p2to3omelette) to[out=190,in=180] (p3to4);

\draw[-o] (p1to2soup) to[out=0,in=170] (p3merge);
\draw[-o] (p3to4) to[out=0,in=-170] (p3merge);

\draw[->] (p3merge) to[out=10,in=180] node[above left]{?} (p4t);
\draw[->] (p3merge) to[out=-10,in=180] node[above]{?} (p24t);
