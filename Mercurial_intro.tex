...
\colorlet{Milk}{green!20}
\colorlet{Soup}{blue!20}
\colorlet{Omelette}{red!20}
\colorlet{Tee}{yellow!20}
\colorlet{Merge}{violet!20}
...
\node<1->[recept, fill=Milk, below=-\pgflinewidth of p1t] (p1milk) {Молоко};
\node<1->[recept_b, fit=(p1t) (p1milk)] {};

\node<3->[recept_t, right of=p1t] (p2t) {\textbf{Меню}};
\node<3->[recept, fill=Milk, below=-\pgflinewidth of p2t] (p2milk) {Молоко};
\node<3->[recept, fill=Soup, below=-\pgflinewidth of p2milk] (p2soup) {Суп};
\node<3->[recept_b, fit=(p2t) (p2soup)] {};

\node<5->[recept_t, right of=p2t] (p3t) {\textbf{Меню}};
\node<5->[recept, fill=Milk, below=-\pgflinewidth of p3t] (p3milk) {Молоко};
\node<5->[recept, fill=Soup, below=-\pgflinewidth of p3milk] (p3soup) {Суп};
\node<5->[recept, fill=Omelette, below=-\pgflinewidth of p3soup] (p3omelette) {Омлет};
\node<5->[recept_b, fit=(p3t) (p3omelette)] {};

\node<7->[recept_t, right of=p3t] (p4t) {\textbf{Меню}};
\node<7->[recept, fill=Tee, below=-\pgflinewidth of p4t] (p4tee) {Чай};
\node<7->[recept, fill=Soup, below=-\pgflinewidth of p4tee] (p4soup) {Суп};
\node<7->[recept, fill=Omelette, below=-\pgflinewidth of p4soup] (p4omelette) {Омлет};
\node<7->[recept_b, fit=(p4t) (p4omelette)] {};

\node<2->[recept, rounded corners, fill=Soup, below] (p1to2soup) at (1.75cm, -2.5cm) {$+$ Суп};
\node<4->[recept, rounded corners, fill=Omelette, below] (p2to3omelette) at (5.25cm, -2.5cm)  {$+$ Омлет};
\node<6->[recept, rectangle split, rectangle split parts=2, rectangle split part fill={Milk, Tee}, rounded corners, below] (p3to4) at (8.75cm, -2.5cm)  {$-$ Молоко \nodepart{two} $+$ Чай};
...
\node[recept, fill=Milk, below=-\pgflinewidth of p1t] (p1milk) {Молоко};
\node[recept_b, fit=(p1t) (p1milk)] {};

\node[recept, rounded corners, fill=Soup, right of=p1t, yshift=0.5cm] (p1to2soup) {$+$ Суп};
\node[recept, rounded corners, fill=Omelette, above=0.3cm of p1to2soup] (p2to3omelette) {$+$ Омлет};
\node[recept, rectangle split, rectangle split parts=2, rectangle split part fill={Milk, Tee}, rounded corners, above=0.3cm of p2to3omelette] (p3to4) {$-$ Молоко \nodepart{two} $+$ Чай};

\node[recept_t, right of=p3to4, yshift=0.5cm] (p4t) {\textbf{Меню}};
\node[recept, fill=Tee, below=-\pgflinewidth of p4t] (p4tee) {Чай};
\node[recept, fill=Soup, below=-\pgflinewidth of p4tee] (p4soup) {Суп};
\node[recept, fill=Omelette, below=-\pgflinewidth of p4soup] (p4omelette) {Омлет};
...
\node[commit, fill=Soup, above of=c0] (c1) {};
\node[commit, fill=Omelette, above of=c1] (c2) {};
\coordinate[above of=c2] (pc2);
\fill[fill=Milk] ($(pc2)+(0.15cm,0)$) arc (0:180:0.15cm) -- cycle;
\fill[fill=Tee] ($(pc2)+(0.15cm,0)$) arc (0:-180:0.15cm) -- cycle;
...
\node<2->[recept, fill=Milk, below=-\pgflinewidth of p21t] (p21milk) {Молоко};
\node<2->[recept_b, fit=(p21t) (p21milk)] {};

\node<3->[recept, rounded corners, fill=Omelette, right of=p21t] (p2to3omelette) {$+$ Омлет};
\node<3->[recept, rectangle split, rectangle split parts=2, rectangle split part fill={Milk, Tee}, rounded corners, above=2pt of p2to3omelette] (p3to4) {$-$ Молоко \nodepart{two} $+$ Чай};

\node<4->[recept, rounded corners, fill=Soup, right of=p1t] (p1to2soup) {$+$ Суп};

\node<5->[recept_t, right of=p1to2soup] (p2t) {\textbf{Меню}};
\node<5->[recept, fill=Milk, below=-\pgflinewidth of p2t] (p2milk) {Молоко};
\node<5->[recept, fill=Soup, below=-\pgflinewidth of p2milk] (p2soup) {Суп};
\node<5->[recept_b, fit=(p2t) (p2soup)] {};

\node<5->[recept_t, right of=p2to3omelette] (p22t) {\textbf{Меню}};
\node<5->[recept, fill=Tee, below=-\pgflinewidth of p22t] (p22tee) {Чай};
\node<5->[recept, fill=Omelette, below=-\pgflinewidth of p22tee] (p22omelette) {Омлет};
\node<5->[recept_b, fit=(p22t) (p22omelette)] {};

\node<6->[recept, rounded corners, fill=Merge] (p3merge) at ($(p2soup.south)!0.5!(p22t.north)$) {\small Слияние};
...
\node<7->[recept, fill=Tee, below=-\pgflinewidth of p24t] (p24tee) {Чай};
\node<7->[recept, fill=Omelette, below=-\pgflinewidth of p24tee] (p24omelette) {Омлет};
\node<7->[recept, fill=Soup, below=-\pgflinewidth of p24omelette] (p24soup) {Суп};
...
\node[recept, rounded corners, fill=Soup, right=-0.5cm of p1t, yshift=1cm] (p1to2soup) {$+$ Суп};
\node[recept, rounded corners, fill=Merge, above=2.8cm of p1t] (merge) {Слияние};

\node[recept, rounded corners, fill=Omelette, left=-0.5cm of p1t, yshift=1cm] (p2to3omelette) {$+$ Омлет};
\node[recept, rectangle split, rectangle split parts=2, rectangle split part fill={Milk, Tee}, rounded corners, above=0.3cm of p2to3omelette] (p3to4) {$-$ Молоко \nodepart{two} $+$ Чай};
...
\begin{tikzpicture}[node distance=1cm]
\node[commit, fill=white] (c0) {};
\node[commit, fill=Soup, above of=c0, xshift=1cm] (c1) {};
\node[commit, fill=Omelette, above of=c0, xshift=-1cm] (c2) {};
\coordinate[above of=c2] (pc2);
\fill[fill=Milk] ($(pc2)+(0.15cm,0)$) arc (0:180:0.15cm) -- cycle;
\fill[fill=Tee] ($(pc2)+(0.15cm,0)$) arc (0:-180:0.15cm) -- cycle;
\node[commit] (c3) at (pc2) {};
\node[commit, fill=Merge, above of=c3, xshift=1cm] (c4) {};
\draw[->] (c0) -- (c1);
\draw[->] (c0) -- (c2);
\draw[->] (c2) -- (c3);
\draw[->] (c3) -- (c4);
