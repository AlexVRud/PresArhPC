...
\begin{tikzpicture}
  ...
  \node[databox=4,text=brown, below=2.5cm of EAX1] (EAX2) at (0,0) {\only<1,2>{123}\only<3->{1024}};
  \node[left=0cm of EAX2] (LabelEAX2) {EAX};
  \node[state, fit=(EAX2) (LabelEAX2)] (State2) {};
  \node[command] (cmd) at($(State1)!0.5!(State2)$)  {\cl{mov eax, \only<1,2>{123}\only<3->{1024}}};
  \draw[-o] (State1) -- (cmd);
  \draw[->] (cmd) -- (State2);
\end{tikzpicture}
        \end{center}
...
\begin{tikzpicture}
  \node[databox=2] (EA16) at (0,0) {};
  \node[databox=1,text=brown,right=-\pgflinewidth of EA16] (AH){\only<2>{0}\only<4>{4}};
  \node[databox=1,text=brown,right=-\pgflinewidth of AH] (AL) {\only<2>{123}\only<4>{0}};
  \node<2,4>[text=brown, xshift=0.5*\ByteWidht] at (0,0) {0};
  \node<2,4>[text=brown, xshift=-0.5*\ByteWidht] at (0,0) {0};
  \node[above=0cm of AH] {AH};
  \node[above=0cm of AL] {AL};
  \draw [decorate,decoration={brace,amplitude=5pt,mirror,raise=1pt}]
...
\only<4>{\footnotesize $$123=0\cdot 256^3+0\cdot 256^2+4\cdot 256^1+0\cdot 256^0$$}
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
%% TODO: Разбор примеров. Показать значения регистров EAX,AX,AH,AL до и после
