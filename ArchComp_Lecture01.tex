...
\tikzset{
  ...
  state/.style={rectangle, draw=gray, very thick, rounded corners},
  command/.style={ellipse, draw=gray, very thick, fill=blue!20!white},
  myArrow/.style={
    draw=gray,
    very thick,
    >=angle 90,
...
%%
%%
\begin{frame}\frametitle{Машинный код}
  \begin{block}{}
    \textbf{Машинный код} или \textbf{машинный язык} представляет собой набор инструкций, выполняемых непосредственно центральным процессором компьютера (CPU). 
  \end{block}
  \begin{block}{}
...
\node[databox=1,text=brown] at (0,0) {0x50};
          \end{tikzpicture}}
          \subfigure{\begin{tikzpicture}
\node[databox=2, text=brown] at (0,0) {0x01D8};
          \end{tikzpicture}}
        \end{figure}
  \end{exampleblock}  
\end{frame}
...
%%
%%
\begin{frame}\frametitle{Низкоуровневый язык программирования}
  \begin{block}{}
\textbf{Низкоуровневый язык программирования} (язык программирования низкого уровня) --- язык программирования, близкий к программированию непосредственно в машинных кодах используемого реального или виртуального (например, Java, Microsoft .NET) процессора.
  \end{block}
  \begin{block}{}
Для обозначения машинных команд обычно применяется мнемоническое обозначение. Это позволяет запоминать команды не в виде последовательности двоичных нулей и единиц, а в виде осмысленных сокращений слов человеческого языка (обычно английских).

[\url{http://dic.academic.ru/dic.nsf/ruwiki/1219317}]
  \end{block}
\end{frame}
...
%%
%%
\begin{frame}\frametitle{Язык ассемблера}
  \begin{block}{}
\textbf{Язык ассемблера} --- машинно-ориентированный язык низкого уровня с командами, не всегда соответствующими командам машины, который может обеспечить дополнительные возможности вроде макрокоманд; автокод, расширенный конструкциями языков программирования высокого уровня, такими как выражения, макрокоманды, средства обеспечения модульности программ.
  \end{block}
  \begin{block}{}
\textbf{Автокод} --- язык программирования, предложения которого по своей структуре в основном подобны командам и обрабатываемым данным конкретного машинного языка.
  \end{block}
  \begin{block}{}
Перевод программы на языке ассемблера в исполнимый машинный код (вычисление выражений, раскрытие макрокоманд, замена мнемоник собственно машинными кодами и символьных адресов на абсолютные или относительные адреса) производится \textbf{ассемблером} --- программой-транслятором, которая и дала языку ассемблера его название.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%% Представление целых знаковых чисел в памяти
%%
\begin{frame}{Представление целых знаковых чисел в памяти}
  \begin{columns}[T]
    \begin{column}[]{0.45\textwidth}  
      \begin{exampleblock}<1->{Как запомнить}
        \begin{center}
\begin{tikzpicture}
  \node[databox=1,text=brown,draw=gray,fill=lightgray,dashed] (Ap24) {\footnotesize N};
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap24] (Ap23) {\footnotesize 0x00}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap23] (Ap22) {\footnotesize 0x00}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap22] (Ap21) {\footnotesize 0x00}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap21] (Ap20) {\footnotesize 0x02}; 
  \node[left=0cm of Ap24] {$2$};

  \node[databox=1,text=brown,draw=gray,fill=lightgray,dashed,below=0.1cm of Ap24] (Ap14) {\footnotesize N};
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap14] (Ap13) {\footnotesize 0x00}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap13] (Ap12) {\footnotesize 0x00}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap12] (Ap11) {\footnotesize 0x00}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap11] (Ap10) {\footnotesize 0x01}; 
  \node[left=0cm of Ap14] {$1$};

  \node[databox=1,text=brown,draw=gray,fill=lightgray,dashed,below=0.1cm of Ap14] (Ap04) {\footnotesize N};
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap04] (Ap03) {\footnotesize 0x00}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap03] (Ap02) {\footnotesize 0x00}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap02] (Ap01) {\footnotesize 0x00}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Ap01] (Ap00) {\footnotesize 0x00}; 
  \node[left=0cm of Ap04] {$0$};

  \node[databox=1,text=brown,draw=gray,fill=lightgray,dashed,below=0.1cm of Ap04] (Am14) {\footnotesize N-1};
  \node[databox=1,text=brown,right=-\pgflinewidth of Am14] (Am13) {\footnotesize 0xFF}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Am13] (Am12) {\footnotesize 0xFF}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Am12] (Am11) {\footnotesize 0xFF}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Am11] (Am10) {\footnotesize 0xFF}; 
  \node[left=0cm of Am14] {$-1$};

  \node[databox=1,text=brown,draw=gray,fill=lightgray,dashed,below=0.1cm of Am14] (Am24) {\footnotesize N-1};
  \node[databox=1,text=brown,right=-\pgflinewidth of Am24] (Am23) {\footnotesize 0xFF}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Am23] (Am22) {\footnotesize 0xFF}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Am22] (Am21) {\footnotesize 0xFF}; 
  \node[databox=1,text=brown,right=-\pgflinewidth of Am21] (Am20) {\footnotesize 0xFE}; 
  \node[left=0cm of Am24] {$-2$};
  
  \node[above=0.2cm of Ap22] {...};
  \node[below=0.2cm of Am22] {...};
  \node[right=0.1cm of Ap20] {\texttt{{\color{gray}7}0002}};
  \node[right=0.1cm of Ap10] {\texttt{{\color{gray}7}0001}};
  \node[right=0.1cm of Ap00] {\texttt{{\color{gray}7}0000}};
  \node[right=0.1cm of Am10] {\texttt{{\color{gray}6}9999}};
  \node[right=0.1cm of Am20] {\texttt{{\color{gray}6}9998}};
\end{tikzpicture}
    ...
    \end{column}
    \begin{column}[]{0.45\textwidth}  
      \begin{block}<1->{К прочтению}
        \begin{itemize}
          \item А.В. Столяров, Программирование на языке ассемблера NASM для ОС Unix
          \begin{itemize}
            \item \url{http://www.stolyarov.info/books/asm_unix}
            \item Глава 1.3. Машинное представление целых чисел.
          \end{itemize}
        \end{itemize}
      \end{block}
    \end{column}
...
\begin{tikzpicture}
  ...
  \node<3->[left=0cm of ECX1] (LabelECX1) {\only<3,4>{ECX}\only<5-8>{n}};
  \node<1,2>[state, fit=(EAX1) (LabelEAX1)] (State1) {};
  \node<3->[state, fit=(EAX1) (LabelEAX1) (ECX1)] (State1) {};

  \node<1-2>[databox=4,text=brown, below=2.5cm of EAX1] (EAX2) {\only<1>{?}\only<2>{379}};
  \node<3->[databox=4,text=brown, below=2.5cm of ECX1] (EAX2) {\only<3,5,7>{?}\only<4>{2256}\only<6>{567014608}\only<8>{2256}};
  \node[left=0cm of EAX2] (LabelEAX2) {EAX};
  \node<3->[databox=4,text=brown, below=0.4cm of EAX2] (ECX2) {\only<3,5,7>{?}\only<4,6,8>{2000}};
  \node<3->[left=0cm of ECX2] (LabelECX2) {\only<3,4>{ECX}\only<5-8>{n}};
  \node<1,2>[state, fit=(EAX2) (LabelEAX2)] (State2) {};
  \node<3->[state, fit=(EAX2) (LabelEAX2) (ECX2)] (State2) {};

  \node[command] (cmd) at($(State1.south)!0.5!(State2.north)$) {\cl{add eax, \only<1,2>{123}\only<3,4>{ecx}\only<5,6>{n}\only<7,8>{[n]}}};
  \draw[->,myArrow] (State1) -- (cmd);
  \draw[->,myArrow] (cmd) -- (State2);
\end{tikzpicture}
        \end{center}
    ...
    \begin{column}[]{0.45\textwidth}  
      ...
      \begin{alertblock}{Внимание!!!}
        Тут происходит загрузка адреса <<переменной>> \cl{n} в \cl{EAX}.
      \end{alertblock}
      }
    \end{column}
  \end{columns}
...
\begin{frame}\frametitle{Команды языка Ассемблер: push}
  ...
  \begin{block}<1->{}
    \begin{itemize}
      \item Поместить источник в стек.
      \item Источник --- регистр или данные в памяти.
    \end{itemize}
  \end{block}
\end{frame}
...
%% Пример push eax
%%
\begin{frame}{Пример использования команды push}
  \begin{exampleblock}<1->{Пример}
    \begin{center}
\begin{tikzpicture}
  \node[databox=4] (ESP1) at (0,0) {};
  \node[left=0cm of ESP1] (LabelESP1) {ESP};
  \node[databox=4,text=brown, right=2*\ByteWidht of ESP1] (EAX1) {1234};
  \node[left=0cm of EAX1] (LabelEAX1) {EAX};
  \node[databox=6, fill=gray, below=0.4cm of ESP1, xshift=\ByteWidht] (ST11) {};
  \node[databox=4,text=brown, right=-\pgflinewidth of ST11] (ST12) {0};
  \node[linkedbyte,left=-\DeltaWidht-\ByteWidht-\pgflinewidth of ST12] (headofstack1) {};
  \node[left=0cm of ST11] (LabelST1) {Stack:};
  \node[state, fit=(LabelESP1) (EAX1) (LabelST1) (ST11)] (State1) {};
  \draw[->,draw=violet,thick] (ESP1.center) -- (headofstack1.north west);


  \node[databox=4,text=brown, below=3.0cm of ESP1] (ESP2) {};
  \node[left=0cm of ESP2] (LabelESP2) {ESP};
  \node[databox=4,text=brown, right=2*\ByteWidht of ESP2] (EAX2) {1234};
  \node[left=0cm of EAX2] (LabelEAX2) {EAX};
  \node[databox=2, fill=gray, below=0.4cm of ESP2, xshift=-\ByteWidht] (ST21) {};
  \node[databox=4,text=brown, right=-\pgflinewidth of ST21] (ST22) {1234};
  \node[databox=4,text=brown, right=-\pgflinewidth of ST22] (ST23) {0};
  \node[linkedbyte,left=-\DeltaWidht-\ByteWidht-\pgflinewidth of ST22] (headofstack2) {};
  \node[left=0cm of ST21] (LabelST2) {Stack:};
  \node[state, fit=(LabelESP2) (EAX2) (LabelST2) (ST21)] (State2) {};
  \draw[->,draw=violet,thick] (ESP2.center) -- (headofstack2.north);

  \node[command] (cmd) at($(State1.south)!0.5!(State2.north)$) {\cl{push \only<1>{eax}}};
  \draw[->,myArrow] (State1) -- (cmd);
  \draw[->,myArrow] (cmd) -- (State2);
\end{tikzpicture}
    \end{center}
...
\begin{frame}\frametitle{Команды языка Ассемблер: pop}
  ...
  \begin{block}<1->{}
    \begin{itemize}
      \item Взять элемент из стека и поместить его в приёмник. 
    \end{itemize}
  \end{block}
\end{frame}
...
%% Пример pop eax
%%
\begin{frame}{Пример использования команды pop}
  \begin{exampleblock}<1->{Пример}
    \begin{center}
\begin{tikzpicture}
  \node[databox=4] (ESP1) at (0,0) {};
  \node[left=0cm of ESP1] (LabelESP1) {ESP};
  \node[databox=4,fill=gray, right=2*\ByteWidht of ESP1] (EAX1) {};
  \node[left=0cm of EAX1] (LabelEAX1) {EAX};
  \node[databox=2, fill=gray, below=0.4cm of ESP1, xshift=-\ByteWidht] (ST11) {};
  \node[databox=4,text=brown, right=-\pgflinewidth of ST11] (ST12) {1234};
  \node[databox=4,text=brown, right=-\pgflinewidth of ST12] (ST13) {0};
  \node[linkedbyte,left=-\DeltaWidht-\ByteWidht-\pgflinewidth of ST12] (headofstack1) {};
  \node[left=0cm of ST11] (LabelST1) {Stack:};
  \node[state, fit=(LabelESP1) (EAX1) (LabelST1) (ST11)] (State1) {};
  \draw[->,draw=violet,thick] (ESP1.center) -- (headofstack1.north);
  ...
  \node[left=0cm of EAX2] (LabelEAX2) {EAX};
  \node[databox=2, fill=gray, below=0.4cm of ESP2, xshift=-\ByteWidht] (ST21) {};
  \node[databox=1, fill=gray, text=white, right=-\pgflinewidth of ST21] (ST22) {210};
  \node[databox=1, fill=gray, text=white, right=-\pgflinewidth of ST22] (ST23) {4};
  \node[databox=1, fill=gray, text=white, right=-\pgflinewidth of ST23] (ST24) {0};
  \node[databox=1, fill=gray, text=white, right=-\pgflinewidth of ST24] (ST25) {0};
  \node[databox=4, text=brown, right=-\pgflinewidth of ST25] (ST26) {0};
  \node[linkedbyte,left=-\DeltaWidht-\ByteWidht-\pgflinewidth of ST26] (headofstack2) {};
  \node[left=0cm of ST21] (LabelST2) {Stack:};
  \node[state, fit=(LabelESP2) (EAX2) (LabelST2) (ST21)] (State2) {};
  \draw[->,draw=violet,thick] (ESP2.center) -- (headofstack2.north west);

  \node[command] (cmd) at($(State1.south)!0.5!(State2.north)$) {\cl{pop eax}};
  \draw[->,myArrow] (State1) -- (cmd);
  \draw[->,myArrow] (cmd) -- (State2);
\end{tikzpicture}
    \end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%% Команды языка Ассемблер: call
%%
\begin{frame}\frametitle{Команды языка Ассемблер: call}
  \begin{block}<1->{Прототип}
    \cl{call \{адрес\}}
  \end{block}
  \begin{block}<1->{}
    \begin{itemize}
      \item Команда безусловного перехода на заданный адресс.
      \item Адрес следующей инструции (регистр EIP) сохраняется в стек. 
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}\frametitle{Команды языка Ассемблер: call}
  ...
  \begin{block}<1->{}
    \begin{itemize}
      \item Команда возврата из подпрограммы.
      \item Адрес должен лежать на вершине стека.
      \item Если задан дополнительный параметр, то перед выходом соответствующее число байт удаляется из стека, и только затем берётся адрес возврата.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]\frametitle{Вызов подпрограммы}
  ...
  \begin{figure}
    \begin{tikzpicture}
\node[code,scale=0.70] (code) at (0,0) {%
  \begin{minipage}{0.9\textwidth}
    \begin{lstlisting}[]
        push    ecx     ; <--- y
        push    ebx     ; <--- x
        call    _my_sum ; <--- my_sum(x,y)
...
\foreach \yy in {1,...,13} {
  \coordinate (l\yy) at (-8pt,-0.70*\yy*\baselineskip+0.5*0.70*\baselineskip);
}
...
%
\begin{scope}[xshift=-1*\ByteWidht,yshift=1*\ByteHeight,scale=0.70, every node/.style={scale=0.70}]
  \node[partleft] (tmp) at (0,0) {...};
  \only<1,9>{%
  ...
  \only<8>{%
    \node[databox=5,fill=lightgray,right=-\pgflinewidth of tmp] (tmp) {};
    \node[databox=4,fill=lightgray,right=-\pgflinewidth of tmp] (tmp) {return ptr};
    \node[databox=4,right=-\pgflinewidth of tmp] (data2) {32};
    \node[databox=4,right=-\pgflinewidth of data2] (data1) {43};
    \node[databox=1,right=-\pgflinewidth of data1] (data) {?};
  }
...
\end{scope}
%
\only<1>{\draw [<-,draw=violet,thick](l1)to[out=180,in=-90] ++(-0.4cm,0.4cm) -- ($(l1)+(-0.4cm,0.7cm)$) to[out=90,in=-90] (EIP);}
\only<2>{\draw [<-,draw=violet,thick](l3)to[out=180,in=-90] ++(-0.4cm,0.4cm) -- ($(l1)+(-0.4cm,0.7cm)$) to[out=90,in=-90] (EIP);}
\only<3>{\draw [<-,draw=violet,thick](l8)to[out=180,in=-90] ++(-0.4cm,0.4cm) -- ($(l1)+(-0.4cm,0.7cm)$) to[out=90,in=-90] (EIP);}
\only<4>{\draw [<-,draw=violet,thick](l9)to[out=180,in=-90] ++(-0.4cm,0.4cm) -- ($(l1)+(-0.4cm,0.7cm)$) to[out=90,in=-90] (EIP);}
\only<5>{\draw [<-,draw=violet,thick](l11)to[out=180,in=-90] ++(-0.4cm,0.4cm) -- ($(l1)+(-0.4cm,0.7cm)$) to[out=90,in=-90] (EIP);}
\only<6>{\draw [<-,draw=violet,thick](l12)to[out=180,in=-90] ++(-0.4cm,0.4cm) -- ($(l1)+(-0.4cm,0.7cm)$) to[out=90,in=-90] (EIP);}
\only<7>{\draw [<-,draw=violet,thick](l13)to[out=180,in=-90] ++(-0.4cm,0.4cm) -- ($(l1)+(-0.4cm,0.7cm)$) to[out=90,in=-90] (EIP);}
\only<8>{\draw [<-,draw=violet,thick](l4)to[out=180,in=-90] ++(-0.4cm,0.4cm) -- ($(l1)+(-0.4cm,0.7cm)$) to[out=90,in=-90] (EIP);}
\only<9>{\draw [<-,draw=violet,thick](l5)to[out=180,in=-90] ++(-0.4cm,0.4cm) -- ($(l1)+(-0.4cm,0.7cm)$) to[out=90,in=-90] (EIP);}
%
\only<3,4,5,6,7>{\draw [->,draw=violet,thick] (rptr) to[out=-90,in=0] ++(-0.5*\ByteWidht,-0.75*\ByteHeight)
		 -- ++(-0.70*5.5*\ByteWidht,0) to[out=180,in=180] (l4);
}
\only<8>{\draw [->,draw=violet,dotted] (rptr) to[out=-90,in=0] ++(-0.5*\ByteWidht,-0.75*\ByteHeight)
		 -- ++(-0.70*5.5*\ByteWidht,0) to[out=180,in=180] (l4);
}
%
\only<1,9>{\draw [->,draw=violet,thick] (ESP) to[out=-90,in=90] (headofstack);}
\only<2,3,4,5,6,7,8>{\draw [->,draw=violet,thick] (ESP) to[out=-90,in=0] ++(-0.5*\ByteWidht,-\ByteHeight)
...
\only<5>{\draw [->,draw=blue,dotted] (data2) to[out=135,in=-45] (EAX);}
\only<5>{\draw [->,draw=blue,dotted] (data1) to[out=135,in=-45] (EBX);}
...
%
\begin{pgfonlayer}{background}
  \begin{scope}[scale=0.70, every node/.style={scale=0.70, text width=0.9\textwidth}]
    \only<2>{\qahigh{1,2}}
    \only<3>{\qahigh{3}}
    \only<4>{\qahigh{8}}
    \only<5>{\qahigh{9,10}}
...
%% TODO: Слайд сократи код предыдущего примера
