\documentclass[10pt,pdf,ignorenonframetext,aspectratio=169]{beamer}
...
\usepackage{hyperref}
\hypersetup{unicode=true,pdfpagemode=FullScreen}
...
\only<2>{
        \draw[scale=0.6,draw=red] (3pt+16*10pt,-2pt-24*20pt) rectangle ++(8*10pt+4pt,-2pt-1*16pt);
        \draw[scale=0.6,draw=red] (4pt+51*10pt,-2pt-24*20pt) rectangle ++(10*10pt+4pt,-2pt-1*16pt);
};
      \end{tikzpicture}
    \end{figure}
  \end{block}
...
%% TODO: Разбор примеров. Показать значения регистров EAX,AX,AH,AL после "mov eax,123" и аналогичных
